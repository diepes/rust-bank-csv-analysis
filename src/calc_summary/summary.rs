                ...
                match &sign_locks[idx] {
                    ...
                    Some(state) if state.expected_positive != is_positive => {
                        return Err(anyhow!(
                            ...
                            compiled[idx].name,
                            if state.expected_positive { "positive" } else { "negative" },
                            state.first_file,
                            state.first_line,
...
mod tests {
    ...
    fn summarizes_matching_categories_for_apr_to_may_window() {
        let txs = vec![
            tx("1", NaiveDate::from_ymd_opt(2025, 4, 2).unwrap(), -120.0, "AUTOMATIC PAYMENT", "Power Co", "", "u1"),
            tx("1", NaiveDate::from_ymd_opt(2025, 5, 15).unwrap(), -500.0, "AUTOMATIC PAYMENT", "ABC Mortgage", "mortgage interest", "u2"),
            tx("1", NaiveDate::from_ymd_opt(2025, 6, 1).unwrap(), -220.0, "AUTOMATIC PAYMENT", "Power Co", "", "u3"),
        ];
    ...
    fn tracks_unmatched_transactions_in_no_match() {
        let txs = vec![tx("1", NaiveDate::from_ymd_opt(2025, 4, 10).unwrap(), -75.0, "PAYMENT", "Unknown Vendor", "misc expense", "u1")];
    ...
    fn marks_only_matching_transactions_for_period() {
        let txs = vec![
            tx("1", NaiveDate::from_ymd_opt(2025, 4, 2).unwrap(), -120.0, "AUTOMATIC PAYMENT", "Power Co", "", "u1"),
            tx("1", NaiveDate::from_ymd_opt(2025, 4, 3).unwrap(), -60.0, "AUTOMATIC PAYMENT", "Unknown", "", "u2"),
            tx("1", NaiveDate::from_ymd_opt(2025, 6, 1).unwrap(), -20.0, "AUTOMATIC PAYMENT", "Power Co", "", "u3"),
        ];
    ...
    fn matches_transactions_outside_period_when_requested() {
        let txs = vec![
            tx("1", NaiveDate::from_ymd_opt(2025, 4, 2).unwrap(), -120.0, "AUTOMATIC PAYMENT", "Power Co", "", "u1"),
            tx("1", NaiveDate::from_ymd_opt(2025, 6, 1).unwrap(), -20.0, "AUTOMATIC PAYMENT", "Power Co", "", "u2"),
        ];
    ...
    fn errors_when_summary_matches_mixed_signs() {
        let mut t1 = tx("1", NaiveDate::from_ymd_opt(2025, 4, 2).unwrap(), -120.0, "PAYMENT", "Power Co", "", "u1");
        t1.source_file = "file_a.csv".into();
        t1.source_line = 8;
        let mut t2 = tx("1", NaiveDate::from_ymd_opt(2025, 4, 3).unwrap(), 90.0, "PAYMENT", "Power Co", "", "u2");
        ...
        let err = summarize_for_period(&[t1, t2], start, end, &definitions)
            .unwrap_err()
            .to_string();
